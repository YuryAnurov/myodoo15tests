...
https://www.youtube.com/watch?v=IL2GUBaAHMk&list=PLqRRLx0cl0hoZM788LH5M8q7KhiXPyuVU&index=44
    ...
    этого поля видно только после нажатия Save
    но если добавить опцию 
            options="{'collaborative': true}"/>
    то переключаясь между вклдаками с разными пользователями можно увидеть результаты изменения без сохранения
    Причем сначала не работало, потом работало с задержкой, а потом начало работать многовенно - без перезагрузки
    страницы. У одоомейт по началу тоже была ошибка
